# Mission Popover, Disposition Alignment, And Layout Key Polish

GitHub issue: #3

## Goal

Keep mission summaries visually attached to the mission that opened them, center the selected Force Disposition names, and make the official layouts key look like a native app asset rather than a full PDF page.
...
Retain the two native disposition selects, their right-side browser arrows, uppercase typography, dimensions, and interaction behavior. Keep `text-align: center` and make the horizontal padding symmetric so the selected disposition name is optically centered within each card instead of being shifted left by the extra arrow-side padding.
...
- Add a failing Node regression test for symmetric disposition-select padding before changing the CSS.
...
- Verify both disposition selects retain centered text and equal left/right padding at both Playwright viewports.
