...
export const missionReferences = {
  ...
  ], { status: { ru: 'Подразделение, уничтожившее врага, становится освящающим. В конце вашего хода оно может освятить одну не домашнюю цель в пределах досягаемости: поместите в её пределах один из ваших маркеров операции; затем подразделение теряет этот статус.', en: 'A unit that destroys an enemy becomes a consecration unit. At the end of your turn it can consecrate one unconsecrated non-home objective in range: place one of your operation markers within range, then remove that unit status.' } }),
  ...
  'vital-link': mission('vital-link', 'Maintain a marker-backed link through central objectives and seize enemy home ground.', 'Поддерживайте связь через центральные цели маркерами и захватите домашнюю позицию врага.', [
    s('ANY BATTLE ROUND', 'End of your turn',
      ...
      c('For each of your operation markers within range of one of those objectives.', 'Ещё за каждый ваш маркер операции в пределах одной из этих целей.', 1, true, { per: 'marker' })),
    s('SECOND BATTLE ROUND ONWARDS', commandTiming,
  ...
  ], { action: action('Sensor Sweep', 'Сканирование', 'Your Shooting phase.', 'Ваша фаза стрельбы.', 'One friendly unit within range of a central objective.', 'Одно дружественное подразделение в пределах центральной цели.', 'Once per turn.', 'Раз за ход.', 'End of your turn, if the unit controls that objective.', 'В конце вашего хода, если подразделение контролирует цель.', 'Remove one operation marker from the battlefield.', 'Удалите один маркер операции с поля.', ['A unit cannot start this action if only one operation marker remains.', 'Подразделение не может начать действие, если на поле остался только один маркер операции.']) }),
  ...
  sabotage: mission('sabotage', 'Commit sabotage at multiple objectives, with extra reward in enemy territory.', 'Проводите диверсии у нескольких целей с дополнительной наградой на территории противника.', [
    s('ANY BATTLE ROUND', 'End of your turn',
      ...
      c("For each of those units within range of an objective in your opponent's territory.", 'Ещё за каждое из этих подразделений в пределах цели на территории противника.', 2, true, { per: 'unit' })),
    s('SECOND BATTLE ROUND ONWARDS', commandTiming, holdOneNonHome(4)),
...
function validateMissionReference(mission) {
  ...
  for (const [sectionIndex, section] of mission.sections.entries()) {
    ...
    for (const [conditionIndex, condition] of section.conditions.entries()) {
      ...
      if ('per' in condition && !['marker', 'objective', 'terrain-area', 'unit'].includes(condition.per)) throw new Error(`${context} has invalid per`);
